...
class TextGenerationError(RuntimeError):
    """Raised when a caller requires real model output and generation fails."""
...
SYSTEM_PROMPTS = {
    ...
    "major_takeaways": "You are a financial analyst. Provide 4 major takeaways using exactly these plain-text headers: Revenue Growth:, Gross Profit Margin:, SG&A Expense Margin:, and EBITDA Margin Stability:. Follow each header with 1-2 sentences. Do not use markdown around the headers.",
...
def generate_text_section(
    ...
    Args:
        ...
        strict: Raise TextGenerationError instead of silently using fallback text
    """
    ...
    if not api_key:
        ...
        if strict:
            raise TextGenerationError(f"no API key configured for '{prompt_type}'")
    ...
    try:
        ...
        print(
            "⚠️ Warning: Could not create model client "
            f"({type(e).__name__})."
        )
        if strict:
            raise TextGenerationError(
                f"could not create model client for '{prompt_type}'"
            ) from None
        return _get_fallback_text(prompt_type, company_name)
    ...
    try:
        ...
        if not content or not content.strip():
            ...
            raise ValueError(f"model returned no final content (finish_reason={finish_reason})")
        ...
        print(
            f"❌ Error generating '{prompt_type}' "
            f"({type(e).__name__})."
        )
        if strict:
            raise TextGenerationError(
                f"model did not produce usable output for '{prompt_type}'"
            ) from None
        return _get_fallback_text(prompt_type, company_name)
