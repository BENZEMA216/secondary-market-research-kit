...
def main():
    ...
    if args.generate_text_sections:
        ...
        if 'openai_api_key' not in locals() or not openai_api_key:
            print("Error: Model API key not loaded. Text generation cannot continue.")
            llm_generation_failures.extend(all_text_types)
        else:
            data_for_text_gen = {
            ...
            for text_type in all_text_types:
                ...
                if text_type == "news_summary" and not company_news:
                    ...
                    llm_generation_skipped.append(text_type)
                    continue
                ...
                try:
                    ...
                    # Treat validation fallback as a failed model section rather
                    # than claiming that AI generation succeeded.
                    if text_type == "competitor_analysis" and (not generated_text or len(generated_text.split('.')) < 3):
                         raise ValueError("competitor analysis failed length validation")
                    
                    elif text_type == "major_takeaways" and "Revenue Growth:" not in generated_text:
                         raise ValueError("major takeaways failed required-section validation")

                    elif text_type == "news_summary" and (not generated_text or len(generated_text.split()) < 50):
                        raise ValueError("news summary failed length validation")

                    file_path = os.path.join(text_output_dir, f"{text_type}.txt")
                    with open(file_path, "w", encoding="utf-8") as f:
                        f.write(generated_text)
                    print(f"✅ Successfully generated and saved '{text_type}' to {file_path}")
                    llm_generation_successes.append(text_type)
                    
                except Exception as e:
                    print(f"Error generating text for '{text_type}': {e}")
                    llm_generation_failures.append(text_type)
    ...
    summary_data = {
        ...
        "text_generation": {
            "enabled": args.generate_text_sections,
            "model": openai_model if args.generate_text_sections else None,
            "successful_sections": llm_generation_successes,
            "fallback_sections": llm_generation_failures,
            "skipped_sections": llm_generation_skipped,
        },
        "files_generated": {
    ...
    if args.generate_text_sections and llm_generation_failures:
        print(f"\n" + "="*60)
        print(
            "❌ Financial data was generated, but the model-backed analysis "
            "is incomplete."
        )
        print(f"📁 Partial outputs saved to: {output_dir}")
        print(
            f"📝 Model text status: {len(llm_generation_successes)} generated, "
            f"{len(llm_generation_failures)} fallback, "
            f"{len(llm_generation_skipped)} skipped"
        )
        print("See analysis_summary.json for per-section status.")
        return 2
